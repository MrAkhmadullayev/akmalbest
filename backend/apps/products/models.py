...
class Product(models.Model):
    ...
    brand = models.ForeignKey(
        Brand, on_delete=models.PROTECT, related_name="products", null=True, blank=True, db_index=True
    )
    volume = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Hajmi (masalan: 0.5, 0.7, 1.0)"
    )
    unit = models.CharField(max_length=10, choices=UnitChoices.choices, default=UnitChoices.BOTTLE)
    ...
    selling_price = models.DecimalField(
        max_digits=14, decimal_places=2, validators=[MinValueValidator(Decimal("0"))], help_text="Sotish narxi (UZS)"
    )
