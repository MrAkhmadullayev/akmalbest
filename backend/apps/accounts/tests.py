...
class EffectivePermissionsTests(CacheIsolatedAPITestCase):
    ...
    def test_role_defaults_apply_when_no_overrides(self):
        cashier = User.objects.create_user(
            username='kassir', password=STRONG_PASSWORD, role=UserRole.CASHIER,
        )
    ...
    def test_overrides_beat_role_defaults_in_both_directions(self):
        cashier = User.objects.create_user(
            username='kassir2', password=STRONG_PASSWORD, role=UserRole.CASHIER,
    ...
    def test_super_admin_ignores_negative_overrides(self):
        boss = User.objects.create_user(
            username='boss2', password=STRONG_PASSWORD, role=UserRole.SUPER_ADMIN,
...
class UserApiPermissionsTests(CacheIsolatedAPITestCase):
    ...
    def setUp(self):
        ...
        self.super_admin = User.objects.create_user(
            username='root', password=STRONG_PASSWORD, role=UserRole.SUPER_ADMIN,
        )
    ...
    def test_update_persists_permissions_and_username(self):
        target = User.objects.create_user(
            username='eski', password=STRONG_PASSWORD, role=UserRole.CASHIER,
        )
    ...
    def test_update_can_change_password(self):
        target = User.objects.create_user(
            username='parolchi', password=STRONG_PASSWORD, role=UserRole.CASHIER,
        )
    ...
    def test_update_without_password_keeps_old_one(self):
        target = User.objects.create_user(
            username='tegilmagan', password=STRONG_PASSWORD, role=UserRole.CASHIER,
        )
        response = self.client.patch(
            reverse('users-detail', args=[target.id]),
    ...
    def test_mismatched_passwords_are_rejected(self):
        target = User.objects.create_user(
            username='nomos', password=STRONG_PASSWORD, role=UserRole.CASHIER,
        )
        response = self.client.patch(
            reverse('users-detail', args=[target.id]),
...
class ModuleGuardTests(CacheIsolatedAPITestCase):
    ...
    def setUp(self):
        ...
        self.cashier = User.objects.create_user(
            username='kassir3', password=STRONG_PASSWORD, role=UserRole.CASHIER,
        )
