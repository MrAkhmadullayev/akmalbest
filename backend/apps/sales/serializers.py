...
class SaleItemSerializer(serializers.ModelSerializer):
    class Meta:
        model = SaleItem
        fields = [
            'id', 'product', 'product_name_snapshot', 'barcode_snapshot',
            'purchase_price_snapshot', 'selling_price_snapshot',
            'quantity', 'returned_quantity', 'discount', 'subtotal', 'profit',
        ]
...
class SaleListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'customer', 'customer_name', 'subtotal', 'discount', 'total',
            'profit', 'payment_method', 'status', 'items_count', 'created_at',
        ]
        ...
        for item in value:
            if 'product_id' not in item or 'quantity' not in item:
                raise serializers.ValidationError(
                    "Har bir element uchun product_id va quantity kerak."
                )
    ...
    def validate(self, attrs):
        if attrs['payment_method'] == 'DEBT':
            ...
            if not attrs.get('due_date'):
                raise serializers.ValidationError(
                    {"due_date": "Nasiya savdo uchun muddat kiritilishi shart."}
                )
        return attrs
