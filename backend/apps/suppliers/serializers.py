    ...
    def validate_items(self, value):
        ...
        for item in value:
            if "product_id" not in item or "quantity" not in item or "purchase_price" not in item:
                raise serializers.ValidationError("Har bir element uchun product_id, quantity va purchase_price kerak.")
            try:
                item["product"] = Product.objects.get(pk=item["product_id"])
            except Product.DoesNotExist as err:
                raise serializers.ValidationError(f"Mahsulot topilmadi: {item['product_id']}") from err
