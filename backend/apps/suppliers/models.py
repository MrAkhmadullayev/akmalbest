...
class Purchase(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    supplier = models.ForeignKey(Supplier, on_delete=models.PROTECT, related_name="purchases", db_index=True)
    invoice_number = models.CharField(max_length=100, blank=True)
    total = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal("0"), validators=[MinValueValidator(Decimal("0"))]
    )
    status = models.CharField(max_length=20, choices=StatusChoices.choices, default=StatusChoices.COMPLETED)
...
class SupplierDebt(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    supplier = models.ForeignKey(Supplier, on_delete=models.PROTECT, related_name="debts", db_index=True)
    purchase = models.OneToOneField(Purchase, on_delete=models.PROTECT, related_name="debt", null=True, blank=True)
    original_amount = models.DecimalField(max_digits=14, decimal_places=2, validators=[MinValueValidator(Decimal("0"))])
    ...
    status = models.CharField(
        max_length=20,
        choices=[("ACTIVE", "Faol"), ("PARTIALLY_PAID", "Qisman to'langan"), ("PAID", "To'langan")],
        default="ACTIVE",
        db_index=True,
    )
    notes = models.TextField(blank=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(remaining_amount__gte=0), name="chk_supplier_debt_remaining_non_negative"
            ),
            models.CheckConstraint(condition=models.Q(paid_amount__gte=0), name="chk_supplier_debt_paid_non_negative"),
        ]
...
class SupplierDebtPayment(models.Model):
    ...
    payment_method = models.CharField(
        max_length=10, choices=[("CASH", "Naqd"), ("TRANSFER", "Pul o'tkazma")], default="CASH"
    )
    paid_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="supplier_debt_payments_made"
    )
    notes = models.TextField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
