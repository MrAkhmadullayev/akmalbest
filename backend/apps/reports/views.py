...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        # Current shift sales
        shift_sales = Sale.objects.filter(created_at__gte=shift_start, status=SaleStatus.COMPLETED)
        ...
        today_expenses = Expense.objects.filter(created_at__gte=shift_start).aggregate(total=Sum("amount"))[
            "total"
        ] or Decimal("0")
        ...
        out_of_stock_count = Product.objects.filter(is_active=True, current_stock__lte=0).count()
        ...
        # Low stock products
        low_stock_products = Product.objects.filter(is_active=True, current_stock__lte=F("warning_stock")).order_by(
            "current_stock"
        )[:10]
        low_stock_data = [
            {
...
class CloseShiftView(APIView):
    ...
    def post(self, request):
        ...
        shift_sales = Sale.objects.filter(created_at__gte=shift_start, created_at__lte=now, status=SaleStatus.COMPLETED)

        shift_expenses = Expense.objects.filter(created_at__gte=shift_start, created_at__lte=now)
...
class SalesReportView(APIView):
    ...
    def get(self, request):
        ...
        by_method = qs.values("payment_method").annotate(count=Count("id"), total=Sum("total"))

        # Sales by day
        daily = (
            qs.extra(select={"day": "DATE(created_at)"})
            .values("day")
            .annotate(total=Sum("total"), profit=Sum("profit"), count=Count("id"))
            .order_by("day")
        )
