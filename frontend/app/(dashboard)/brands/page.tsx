...
import { brandsService } from '@/services/products';
...
export default function BrandsPage() {
  ...
  const handleDelete = (id: string) => {
    if (confirm('Ushbu brendni o\'chirmoqchimisiz?')) {
      deleteMutation.mutate(id);
    }
  };

  const brands = data?.data?.results || [];
  ...
  return (
    <div className="p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="card p-6 h-fit space-y-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">
            {editingId ? 'Brendni tahrirlash' : 'Yangi brend qo\'shish'}
          </h2>
          <p className="text-gray-500 text-sm mt-0.5">
            Brend ma&apos;lumotlarini to&apos;ldiring
          </p>
        </div>
      ...
      <div className="card p-6 lg:col-span-2 space-y-4">
        <h2 className="text-xl font-bold text-gray-900">Brendlar ro&apos;yxati</h2>
              ...
              <tbody>
                {brands.map((b) => (
                  <tr key={b.id}>
                    ...
                    <td className="text-gray-500 max-w-xs truncate">{b.description || '-'}</td>
                    <td className="text-center font-semibold">{b.product_count}</td>
                    <td>
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          title={b.product_count > 0 ? "Ushbu brendda mahsulotlar bor" : ""}
                        >
                          <Trash2 size={18} />
                        </button>
                      </div>
